...
#  dbConnectionService=DBConnectionService(settings['dbType'], host=settings['hostname'], port=settings['port'], user=usr, password=passwd, dbName=settings['dbName'])
  ...
  if settings['remainder']=="delete":
    print "deleting now"
#    dbConnectionService.deleteFromDB('RPCPVSSPERLUMI')
#    dbConnectionService.dropFromDB('RPCPVSSPERLUMI')
#    dbConnectionService.deleteFromDB('RPCPVSSTEMPPERRUN')
#    dbConnectionService.dropFromDB('RPCPVSSTEMPPERRUN')
    dbConnectionService.deleteFromDB('TESTOUT_PVSS_ADG')
    dbConnectionService.dropFromDB('TESTOUT_PVSS_ADG')
...
#  dbServicePVSSPERLUMI = runDataToDB_PVSSPERLUMI_PVSSPERRUN_ORACLE_OMDS.DBService(dbConnectionService.engine(),schema)
  if settings['remainder']=="create":
    if  settings['dbType'] == "sqlite:///":
      dbServicePVSSPERLUMISQL.createDBTEMPPERRUN()
    else:
      dbServicePVSSPERLUMI.createDBTEMPPERRUN()
...
#  fileReaderPVSSPERRUN = runDataToDB_PVSSPERRUN_ORACLE_OMDS.FileReaderPVSSPERRUN()
 ...
 #  fileReaderPVSSPERLUMI = runDataToDB_PVSSPERRUN_ORACLE_OMDS.FileReaderPVSSPERLUMI()
  #   fileReaderEFFICIENCY = runDataToDB_EFFICIENCY_ORACLE_OMDS.FileReader()
...
#    dataPVSSPERRUN = fileReaderPVSSPERRUN.read(directory+str(run)+"_ImonPerRun.txt","IMON")
#    dataPVSSPERRUNVMON = fileReaderPVSSPERRUN.read(directory+str(run)+"_VmonPerRun.txt","VMON")

    dataPVSSTEMPPERRUN = fileReaderPVSSPERRUN.readTemp(directory+str(run)+"_TempPerRun.txt")

#    dataPVSSPERLUMI = fileReaderPVSSPERLUMI.read(directory+str(run)+"_ImonPerLumi.txt")
  
    #  dataPVSSPERRUN = fileReaderPVSSPERRUN.read("202389_General.txt")
    #  dataPVSSPERLUMI = fileReaderPVSSPERLUMI.read("202389.txt")
    #  dataPVSSPERRUN = fileReaderPVSSPERRUN.read("206484_General.txt")
    #  dataPVSSPERLUMI = fileReaderPVSSPERLUMI.read("206484.txt")

#    insertList = dbServicePVSSPERLUMI.insertToDB(dataPVSSPERRUN,dataPVSSPERLUMI)
  if settings['dbType'] == "sqlite:///":
    insertList = dbServicePVSSPERLUMISQL.insertTempToDB(dataPVSSTEMPPERRUN)
  else:  
    insertList = dbServicePVSSPERLUMI.insertTempToDB(dataPVSSTEMPPERRUN)
  print insertList
